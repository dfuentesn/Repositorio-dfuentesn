...
void ajuste_lineal(const std::vector<double>& x, const std::vector<double>& y,      //Aquí realizo la regresión lineal
    ...
    double Exx = 0.0, Exy = 0.0, Eyy = 0.0;                                        //Acumulador para los promedios de exx, exy y eyy
    for (size_t i = 0; i < N; ++i) {
        Exx += x[i] * x[i];                                                         //Promedio de las x**2
        Exy += x[i] * y[i];                                                         //Promedio de las x*y
        Eyy += y[i] * y[i];                                                         //Promedio de las y**2 (añadido para Eyy)
    }
    Exx /= N;                                                                       //Retorno los valores a la sumatoria o ecuación 3
    Exy /= N;                                                                       //Lo mismo, ecuación 4
    Eyy /= N;                                                                       //Promedio de y² (necesario para s²)
    ...
    b = (Exx * Ey - Ex * Exy) / denominador;                                        //Aquí estimo el intercepto, ecuación 6
                                                                                    ...
                                                                                    // Cálculo del error cuadrático según ecuación 7 (corregido)
    double s_squared = (N/(N-2.0)) * (Eyy - Ey*Ey - m*m*(Exx - Ex*Ex));
                                                                                    // Cálculo de errores según ecuaciones 8 y 9 (corregido)
    double delta_m_squared = s_squared / (N * (Exx - Ex*Ex));
    delta_m = std::sqrt(delta_m_squared);                                           //Cálculo del error de la pendiente
    delta_b = std::sqrt(delta_m_squared * Exx);                                     //Cálculo del error del intercepto
}
...
void constantes_fisicas(double m, double b) {                                       
    ...
    double phi_J = -b * e;                                                          //Cálculo del módulo de trabajo en JULIOS (unidad del trabajo)
    double phi_V = -b;                                                              //Cálculo del módulo de trabajo en Voltios (Como daría según la fórmula y el intercepto pero positivo)
